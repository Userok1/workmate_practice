        ...
        async with session.begin():
            ...
            for i in range(len(table)):
                if table[i][3] in ("-", "nan"):
                    ...
                    volume = int("".join(num_without_colons))
                if table[i][4] in ("-", "nan"):
                    total = None
                else:
                    num_without_colons = table[i][4].split(",")
                    total = int("".join(num_without_colons))
                if table[i][-1] in ("-", "nan"):
                    count = None
                else:
                    num_without_colons = table[i][-1].split(",")
                    count = int("".join(num_without_colons))

                mappings.append({
                    "exchange_product_id": table[i][0],
                    "exchange_product_name": table[i][1],
                    "oil_id": table[i][0][:4],
                    "delivery_basis_id": table[i][0][4:7],
                    "delivery_basis_name": table[i][2],
                    "delivery_type_id": table[i][0][-1],
                    "volume": volume,
                    "total": total,
                    "count": count,
                    "date": date
                })
            await session.execute(insert(TradeResultsOrm), mappings)
...
async def _load_first_page(page: Page, date: DatetimeDate, session_factory: async_sessionmaker[AsyncSession]) -> None:
    ...
    table: Pdf = (await asyncio.to_thread(page.extract_tables))[-1]
    if not table:
        raise TableNotFoundToExtract("Table for extraction not found")
    ...
    async with HTTP_SEMAPHORE:
        try:
            ...
            try:
                ...
                pdf_parsed = await asyncio.to_thread(pdfplumber.open, pdf_doc_response_bytes)
            except Exception as e:
                logger.error(f"pdf parse error: {str(e)}")
                return
            end = time.time()

            logger.info(f"pdfplumber's open implements in {end - start}s")
            page_num: int = 0
                ...
                if tasks:
                    for task in tasks:
                        if isinstance(task, Exception):
                            logger.error(f"Error in task: {str(task)}")
                            continue
                if is_end:
                    break
